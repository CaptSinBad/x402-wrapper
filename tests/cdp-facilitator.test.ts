...
describe('settleCDPPayment', () => {
    it('should attempt settlement and return a specific error (not generic Error)', async () => {
        ...
        // 3. Assertions
        if (!result.success) {
            expect(result.errorReason).not.toBe('Error');
            expect(result.errorReason).not.toBe('CDP settle failed');
            if (result.errorReason?.includes('insufficient_funds')) {
                console.log('Got expected insufficient_funds error');
            }
        }
    }, 30000); // 30s timeout
});
